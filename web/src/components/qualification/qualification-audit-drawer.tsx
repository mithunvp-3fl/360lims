...
import { useQualificationAudit } from "@/lib/queries";
...
export function QualificationAuditDrawer({
  qualificationNumber,
  open,
  onOpenChange,
}: {
  ...
  const { data } = useQualificationAudit(qualificationNumber);
      ...
      <SheetContent side="right" size="xl" className="!max-w-[640px]">
        <SheetHeader>
          <div className="flex items-center gap-2">
            ...
            <SheetTitle>Audit trail · {qualificationNumber}</SheetTitle>
          </div>
          <SheetDescription>
            Every mutation across the qualification, samples, tests and results. Field-level diffs preserved.
          </SheetDescription>
...
function DiffSection({ log }: { log: AuditLog }) {
  ...
  if (Array.isArray(beforeValues) || Array.isArray(afterValues)) {
    return (
      <ParameterDiff
        before={
          Array.isArray(beforeValues) ? (beforeValues as Array<Record<string, unknown>>) : []
        }
            ...
            {params.map((p) => {
              ...
              return (
                <tr key={p} className="border-t border-line/60 bg-surface">
                  ...
                  <td className="py-1 px-1 text-ink-subtle">
                    <ArrowRight className="h-3 w-3" />
                  </td>
