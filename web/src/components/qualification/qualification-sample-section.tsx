...
import {
  useCreateQualificationSample,
  useQualificationSamples,
  useRecollectQualificationSample,
} from "@/lib/queries";
import { relativeTime, shortDate } from "@/lib/utils";
import type { QualificationSample } from "@/lib/types";

export function QualificationSampleSection({ qualificationNumber }: { qualificationNumber: string }) {
  const { data: samples } = useQualificationSamples(qualificationNumber);
  const create = useCreateQualificationSample(qualificationNumber);
  const recollect = useRecollectQualificationSample(qualificationNumber);

  return (
    <SectionCard
      title="Sample management"
      description="Samples drawn for this qualification and their disposition."
    ...
    >
      {!samples || samples.length === 0 ? (
        <EmptyState
          ...
          description="Click Create Sample to generate a sample ID and auto-assign the qualification test matrix."
        />
      ) : (
...
function SampleCard({ sample }: { sample: QualificationSample }) {
  const tone =
