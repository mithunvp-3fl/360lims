...
import {
  useInstruments,
  useQualificationImportFromInstrument,
  useQualificationSamples,
} from "@/lib/queries";
...
// PRD §18 mandates these six stages with the listed progress percentages
// and a total duration of approximately 5 seconds.
const STAGES: FlowStage[] = [
  ...
  {
    key: "read",
    label: ({ sampleId }) => `Reading batch data for ${sampleId}...`,
...
> = {
  "instrument-offline": {
    ...
    detail: "Heartbeat to the instrument was lost. Check network and driver service.",
  },
  "communication-timeout": {
    failAt: "verify",
    title: "Communication timeout",
    detail: "The instrument did not respond within the protocol window.",
  },
  "invalid-structure": {
    failAt: "parse",
    title: "Invalid result structure",
    detail: "Parsed payload did not match expected schema for this test code.",
  },
};

export function QualificationInstrumentFlow({
  qualificationNumber,
  test,
  open,
  onOpenChange,
  ...
  const { data: samples } = useQualificationSamples(qualificationNumber);
  const importMut = useQualificationImportFromInstrument(qualificationNumber);
  ...
  const sampleDisplayId = sample?.sampleId ?? "PMQS-…";
        ...
        {stageIdx === -1 && !error && (
          <div className="grid grid-cols-2 gap-3 items-end">
            ...
            <div className="text-[11px] text-ink-muted leading-relaxed">
              Total simulation runs ~5 seconds with explicit progress per stage.
            </div>
          </div>
        )}
        ...
        <div className="surface-inset p-4 space-y-2.5">
          {STAGES.map((s, i) => {
            const status: "pending" | "running" | "done" | "failed" =
              error && i >= STAGES.findIndex((st) => st.key === error.failAt)
                ? i === STAGES.findIndex((st) => st.key === error.failAt)
                  ? "failed"
                  : "pending"
                : stageIdx === -1
                  ? "pending"
