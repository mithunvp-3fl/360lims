...
export function QualificationActivityFeed({
  qualificationNumber,
  qualificationId,
}: {
  ...
  const items = (data ?? []).filter((n) => {
    ...
    return (
      (qualificationId && n.entityId === qualificationId) ||
      meta.qualificationNumber === qualificationNumber ||
      (n.entityType?.startsWith("qualification") ?? false) ||
      n.message.toLowerCase().includes(qualificationNumber.toLowerCase()) ||
      n.title.toLowerCase().includes(qualificationNumber.toLowerCase())
    );
  });
  ...
  return (
    <SectionCard
      title="Activity feed"
      description="Live qualification events. Click any entry for details."
      ...
      <div className="relative max-h-[360px] overflow-y-auto -mx-2 px-2">
        {items.length === 0 ? (
          <div className="text-xs text-ink-muted py-6 text-center">
            No events yet for this qualification.
          </div>
        ) : (
...
function inferSource(n: AppNotification): string {
  ...
  if (n.entityType === "qualification") return "Process Engineer";
  if (n.entityType === "qualification-sample") return "Sampler";
  if (n.entityType === "qualification-result") return "Lab Analyst";
  return "System";
}
      ...
      <SheetContent side="right" size="md">
        ...
        <SheetBody>
          <div className="space-y-4">
            ...
            {Array.isArray((meta as Record<string, unknown>).parameters) && (
              <div className="space-y-1.5">
                <div className="text-[10px] font-semibold uppercase tracking-wider text-ink-muted">
                  Parameters captured ({((meta as Record<string, unknown>).parameters as unknown[]).length})
                </div>
                <div className="flex flex-wrap gap-1">
                  {((meta as Record<string, unknown>).parameters as string[]).map((p) => (
