...
import {
  useInstruments,
  useMetalImportFromInstrument,
  useMetalSamples,
} from "@/lib/queries";
import type { MetalTest } from "@/lib/types";
...
// PRD §18 — mandatory 6-stage sequence totalling ~5 seconds with a progress bar.
...
> = {
  "instrument-offline": {
    failAt: "connect",
    title: "OES offline",
    detail: "Heartbeat to the OES was lost. Check spark stand and driver service.",
  },
  "communication-timeout": {
    failAt: "verify",
    title: "Communication timeout",
    detail: "OES did not respond within the protocol window.",
  },
  "invalid-structure": {
    failAt: "parse",
    title: "Invalid chemistry payload",
    detail: "Parsed payload did not match expected element schema.",
  },
};

export function MetalInstrumentFlow({
  metalBatchNumber,
  test,
  open,
  onOpenChange,
  ...
  const { data: samples } = useMetalSamples(metalBatchNumber);
  const importMut = useMetalImportFromInstrument(metalBatchNumber);
  ...
  const instrument = React.useMemo(() => {
    const oesInstruments = (instruments ?? []).filter(
      (i) => i.type === "OES" || i.code.startsWith("OES"),
    );
    if (!test?.instrumentCode) return oesInstruments[0] ?? instruments?.[0];
    return (instruments ?? []).find((i) => i.code === test.instrumentCode) ?? oesInstruments[0] ?? instruments?.[0];
  }, [instruments, test]);
    ...
    importMut.mutate(
      { testId: test.id, instrumentCode: instrument.code },
      {
        onSuccess: () => {
          setDone(true);
          setRunning(false);
          toast.success("OES results imported", {
        ...
        onError: (e: unknown) => {
          ...
          setError({
            failAt: "validate",
            title: "Import failed",
            detail: e instanceof Error ? e.message : "Server rejected the chemistry payload.",
          });
        },
      },
    );
  ...
  const sampleDisplayId = sample?.sampleId ?? "MQS-…";
  const instrumentName = instrument?.name ?? "Thermo OES";
      ...
      <DialogContent className="sm:max-w-[560px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {test ? (
              <>
                Triggering chemistry transfer for <span className="font-medium text-ink">{test.name}</span>{" "}
            ...
            <div className="space-y-1.5">
              ...
              <Select value={failureMode} onValueChange={(v) => setFailureMode(v as FailureMode)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="instrument-offline">OES offline</SelectItem>
                  <SelectItem value="communication-timeout">Communication timeout</SelectItem>
                  <SelectItem value="invalid-structure">Invalid chemistry payload</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="text-[11px] text-ink-muted leading-relaxed">
              Total OES read runs ~5 seconds with explicit progress per stage.
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
          <div className="flex items-center gap-1.5">
            {done && <Badge tone="success">Chemistry captured</Badge>}
