...
import { useMetalManualEntry } from "@/lib/queries";
...
export function MetalManualEntryDialog({
  metalBatchNumber,
  test,
  open,
  onOpenChange,
  ...
  const mutate = useMetalManualEntry(metalBatchNumber);
  ...
  function submit(e: React.FormEvent) {
    ...
    try {
      ...
      mutate.mutate(
        { testId: test.id, reason, values: parsed },
        {
          onSuccess: () => {
            toast.info("Manual chemistry saved", {
              description: `${test.name} (${reason})`,
            });
            onOpenChange(false);
          },
          onError: (err: unknown) =>
      ...
      <DialogContent>
        <form onSubmit={submit} className="space-y-4">
          <DialogHeader>
            ...
            <DialogDescription>
              Capture chemistry outside of an instrument. A reason is mandatory and will be audited.
            </DialogDescription>
          </DialogHeader>
