...
import { MetalInstrumentFlow } from "./metal-instrument-flow";
import { MetalManualEntryDialog } from "./metal-manual-entry-dialog";
import { MetalFileUploadDialog } from "./metal-file-upload-dialog";
import {
  useMetalResults,
  useMetalRetest,
  useMetalTests,
} from "@/lib/queries";
import type { MetalResult, MetalTest, ResultStatus } from "@/lib/types";
import { cn, relativeTime } from "@/lib/utils";
import { resultStatusToAccent } from "@/lib/format";

export function MetalChemistryWorkspace({
  metalBatchNumber,
}: {
  metalBatchNumber: string;
}) {
  const { data: tests } = useMetalTests(metalBatchNumber);
  const { data: results } = useMetalResults(metalBatchNumber);
  const retest = useMetalRetest(metalBatchNumber);

  const [importFor, setImportFor] = React.useState<MetalTest | null>(null);
  const [manualFor, setManualFor] = React.useState<MetalTest | null>(null);
  const [fileFor, setFileFor] = React.useState<MetalTest | null>(null);

  const resultByTestId = React.useMemo(() => {
    const m = new Map<string, MetalResult>();
    for (const r of results ?? []) m.set(r.testId, r);
    return m;
  }, [results]);

  // PRD §19 — show parameter readouts gradually, not all at once.
  ...
  return (
    <SectionCard
      title="Chemistry results workspace"
      description="Capture molten metal chemistry from the OES, manual entry, or uploaded files. Spec checks run on every element."
    ...
    >
      {!tests || tests.length === 0 ? (
        <EmptyState
          ...
          description="Generate a sample to auto-assign the OES test."
        />
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-inset/60 text-[11px] uppercase tracking-wide text-ink-muted">
                ...
                <th className="text-left font-semibold py-2.5 px-3">Chemistry</th>
      ...
      <MetalInstrumentFlow
        metalBatchNumber={metalBatchNumber}
        test={importFor}
        open={!!importFor}
        onOpenChange={(o) => !o && setImportFor(null)}
      />
      <MetalManualEntryDialog
        metalBatchNumber={metalBatchNumber}
        test={manualFor}
        open={!!manualFor}
        onOpenChange={(o) => !o && setManualFor(null)}
      />
      <MetalFileUploadDialog
        metalBatchNumber={metalBatchNumber}
        test={fileFor}
        open={!!fileFor}
...
function ResultSummary({ result }: { result: MetalResult }) {
...
function ParameterReadout({
  ...
  value: MetalResult["values"][number];
  fresh?: boolean;
  staggerIdx?: number;
}) {
  return (
    <div
      ...
      style={
        fresh
          ? {
              animationDelay: `${staggerIdx * 200}ms`,
