...
export function MetalActivityFeed({
  metalBatchNumber,
  metalBatchId,
}: {
  ...
  const items = (data ?? []).filter((n) => {
    ...
    return (
      (metalBatchId && n.entityId === metalBatchId) ||
      meta.metalBatchNumber === metalBatchNumber ||
      (n.entityType?.startsWith("metal-") ?? false) ||
      n.message.toLowerCase().includes(metalBatchNumber.toLowerCase()) ||
      n.title.toLowerCase().includes(metalBatchNumber.toLowerCase())
    );
  });
  ...
  return (
    <SectionCard
      title="Activity feed"
      description="Live metal batch events. Click any entry for details."
      ...
      <div className="relative max-h-[360px] overflow-y-auto -mx-2 px-2">
        {items.length === 0 ? (
          <div className="text-xs text-ink-muted py-6 text-center">
            No events yet for this metal batch.
          </div>
        ) : (
...
function inferSource(n: AppNotification): string {
  ...
  if (n.entityType === "metal-batch") return "Casthouse Operator";
  if (n.entityType === "metal-sample") return "Lab Analyst";
  if (n.entityType === "metal-result") return "Lab Analyst";
  return "System";
}
        ...
        <SheetBody>
          <div className="space-y-4">
            ...
            {Array.isArray((meta as Record<string, unknown>).parameters) && (
              <div className="space-y-1.5">
                <div className="text-[10px] font-semibold uppercase tracking-wider text-ink-muted">
                  Elements captured ({((meta as Record<string, unknown>).parameters as unknown[]).length})
                </div>
