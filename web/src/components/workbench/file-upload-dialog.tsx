...
import { useFileUpload } from "@/lib/queries";
...
export function FileUploadDialog({
  ...
  const upload = useFileUpload(lot);
  const inputRef = React.useRef<HTMLInputElement>(null);
  const [fileName, setFileName] = React.useState<string | null>(null);
  const [phase, setPhase] = React.useState(-1);

  React.useEffect(() => {
    if (!open) { setFileName(null); setPhase(-1); }
  }, [open]);
              ...
              {PHASES.map((p, i) => {
                ...
                return (
                  <div key={p.label} className="flex items-center gap-2.5 text-sm">
                    ...
                    >
                      {done ? <Check className="h-3 w-3" /> : running ? <Loader2 className="h-3 w-3 animate-spin" /> : i + 1}
                    </div>
                    <div className={cn("flex-1", done && "text-ink", running && "text-ink font-medium", !done && !running && "text-ink-muted")}>
                      {p.label}
                    </div>
                  </div>
                );
