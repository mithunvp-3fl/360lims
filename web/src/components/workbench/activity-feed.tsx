...
export function ActivityFeed({ lot, receiptId }: { lot: string; receiptId?: string }) {
  ...
  return (
    <SectionCard
      ...
      description="Live events for this lot. Click any entry for details."
...
function inferSource(n: AppNotification): string {
  if (n.meta?.instrument) return String(n.meta.instrument);
  if (n.meta?.fileName) return `Upload · ${n.meta.fileName}`;
  if (n.entityType === "receipt") return "Stores Executive";
  if (n.entityType === "sample") return "Sampler";
  if (n.entityType === "result") return "Lab Analyst";
  return "System";
}
        ...
        <SheetHeader>
          <div className="flex items-center gap-2">
            ...
            <Badge tone={
              ...
              : notification?.severity === "danger" ? "danger" : "info"
            }>
          ...
          <div className="space-y-4">
            ...
            {Object.keys(meta).length > 0 && (
              <div className="space-y-1.5">
                ...
                <DetailGrid
                  rows={Object.entries(meta).map(([k, v]) => [
                    humanise(k),
                    formatMetaValue(v),
                  ])}
                />
            ...
            {Array.isArray(meta.parameters) && (
              <div className="space-y-1.5">
                <div className="text-[10px] font-semibold uppercase tracking-wider text-ink-muted">
                  Parameters captured ({(meta.parameters as unknown[]).length})
                </div>
                <div className="flex flex-wrap gap-1">
                  {(meta.parameters as string[]).map((p) => (
