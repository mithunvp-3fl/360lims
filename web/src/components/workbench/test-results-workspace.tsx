...
import { InstrumentImportFlow } from "./instrument-import-flow";
import { ManualEntryDialog } from "./manual-entry-dialog";
import { FileUploadDialog } from "./file-upload-dialog";
import { useResults, useRetest, useTests } from "@/lib/queries";
import type { Result, ResultStatus, Test } from "@/lib/types";
import { cn } from "@/lib/utils";
import { resultStatusToAccent } from "@/lib/format";
import { relativeTime } from "@/lib/utils";

export function TestResultsWorkspace({ lot }: { lot: string }) {
  const { data: tests } = useTests(lot);
  const { data: results } = useResults(lot);
  const retest = useRetest(lot);

  const [importFor, setImportFor] = React.useState<Test | null>(null);
  const [manualFor, setManualFor] = React.useState<Test | null>(null);
  const [fileFor, setFileFor] = React.useState<Test | null>(null);
  ...
  return (
    <SectionCard
      ...
      description="Capture results from instruments, manual entry, or uploaded files. Spec checks run on every value."
    ...
    >
      {!tests || tests.length === 0 ? (
        <EmptyState
          ...
          description="Create a sample to auto-assign the tests required by this material's spec."
        />
      ) : (
                ...
                return (
                  <React.Fragment key={t.id}>
                    <tr className="border-t border-line hover:bg-inset/40">
                      ...
                      <td className="py-3 px-3">
                        {r ? <ResultSummary result={r} /> : <span className="text-xs text-ink-subtle">Awaiting capture</span>}
                      </td>
                          ...
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem
                                  onClick={() => retest.mutate(r.id, { onSuccess: () => toast.info("Retest queued") })}
                                >
                        ...
                        <td colSpan={5} className="px-3 py-2.5">
                          ...
                          <div className="flex items-center gap-3 mt-2 text-[11px] text-ink-muted">
                            ...
                            {r.reason && (
                              <>
                                <span>·</span>
                                <span>Reason: <span className="text-ink">{r.reason}</span></span>
                              </>
                            )}
                            {r.fileName && (
                              <>
                                <span>·</span>
                                <span>File: <span className="text-ink">{r.fileName}</span></span>
                              </>
                            )}
                          </div>
                        </td>
      ...
      <InstrumentImportFlow lot={lot} test={importFor} open={!!importFor} onOpenChange={(o) => !o && setImportFor(null)} />
      <ManualEntryDialog lot={lot} test={manualFor} open={!!manualFor} onOpenChange={(o) => !o && setManualFor(null)} />
      <FileUploadDialog lot={lot} test={fileFor} open={!!fileFor} onOpenChange={(o) => !o && setFileFor(null)} />
    </SectionCard>
  );
}

function TestStatusBadge({ result, test }: { result?: Result; test: Test }) {
  if (!result) return <Badge tone="muted">{test.status}</Badge>;
  if (result.overallStatus === "Fail") {
    return <Badge tone="danger"><XCircle className="h-3 w-3" /> Out of spec</Badge>;
  }
  if (result.overallStatus === "Warning") {
    return <Badge tone="warning"><AlertTriangle className="h-3 w-3" /> Variance</Badge>;
  }
  return <Badge tone="success"><CheckCircle2 className="h-3 w-3" /> Compliant</Badge>;
}
...
function ResultSummary({ result }: { result: Result }) {
  ...
  return (
    <div className="text-xs text-ink-muted">
      <span className="text-ink font-medium tabular-nums">{pass}/{total}</span> in spec
