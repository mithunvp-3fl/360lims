...
const ACTION_TONE: Record<string, "info" | "success" | "warning" | "danger" | "accent" | "muted"> = {
  create: "success",
  update: "info",
  delete: "danger",
  import: "accent",
      ...
      <SheetContent side="right" size="xl" className="!max-w-[640px]">
        <SheetHeader>
          <div className="flex items-center gap-2">
            ...
            <SheetTitle>Audit trail · {lot}</SheetTitle>
          </div>
          <SheetDescription>
            Every mutation across the receipt, samples, tests and results for this lot. Field-level
            diffs are preserved.
          </SheetDescription>
    ...
    <div className="flex items-start gap-3 py-2.5">
      ...
      <div className="flex-1 min-w-0 surface-inset p-3 space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          <Badge tone="outline" className="font-mono">{log.entityType}</Badge>
...
function DiffSection({ log }: { log: AuditLog }) {
  ...
  // Special-case: a result import or modification carrying a `values` array of parameter readouts.
  const beforeValues = beforeIsObj ? (before as Record<string, unknown>).values : null;
  const afterValues = afterIsObj ? (after as Record<string, unknown>).values : null;
  if (Array.isArray(beforeValues) || Array.isArray(afterValues)) {
    return (
      <ParameterDiff
        before={Array.isArray(beforeValues) ? (beforeValues as Array<Record<string, unknown>>) : []}
        after={Array.isArray(afterValues) ? (afterValues as Array<Record<string, unknown>>) : []}
      />
    );
  }

  if (beforeIsObj || afterIsObj) {
    return <ObjectDiff before={(before ?? {}) as Record<string, unknown>} after={(after ?? {}) as Record<string, unknown>} />;
  }
          ...
          <tbody>
            {changed.map((k) => (
              <tr key={k} className="border-t border-line/60 bg-surface">
                ...
                <td className="py-1.5 px-1 text-ink-subtle"><ArrowRight className="h-3 w-3" /></td>
...
function ParameterDiff({
  ...
  const beforeMap = new Map(before.map((v) => [String(v.parameter), v]));
  const afterMap = new Map(after.map((v) => [String(v.parameter), v]));
  const params = Array.from(new Set([...beforeMap.keys(), ...afterMap.keys()]));
  return (
    <div className="space-y-1">
      <div className="text-[10px] font-semibold uppercase tracking-wider text-ink-muted">
        Parameter changes
        ...
        <table className="w-full text-[11px]">
          <thead className="bg-surface text-[10px] uppercase text-ink-muted">
            <tr>
              ...
              <th className="text-right font-semibold py-1.5 px-2">Before</th>
              <th className="text-left font-semibold py-1.5 px-1"></th>
              <th className="text-right font-semibold py-1.5 px-2">After</th>
            </tr>
          </thead>
          <tbody>
            {params.map((p) => {
              ...
              return (
                <tr key={p} className="border-t border-line/60 bg-surface">
                  <td className="py-1 px-2 font-medium text-ink">{p}</td>
                  <td className="py-1 px-2 text-right text-danger line-through tabular-nums">
                    {b ? `${formatScalar(b.value)}${b.unit ?? ""}` : "—"}
                  </td>
                  <td className="py-1 px-1 text-ink-subtle"><ArrowRight className="h-3 w-3" /></td>
                  <td className="py-1 px-2 text-right text-success tabular-nums">
                    {a ? `${formatScalar(a.value)}${a.unit ?? ""}` : "—"}
                  </td>
                </tr>
              );
            })}
